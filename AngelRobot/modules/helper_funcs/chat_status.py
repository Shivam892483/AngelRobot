...
def is_user_admin(chat: Chat, user_id: int, member: ChatMember = None) -> bool:
    if (
        ...
        or user_id in [777000, 5242142514]
...
def sudo_plus(func):
    ...
    def is_sudo_plus_func(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if user and is_sudo_plus(chat, user.id):
            ...
            update.effective_message.reply_text(
                "Your are not an admin dear , you want punch?"
            )
...
def bot_admin(func):
    ...
    def is_admin(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            not_admin = "First make me an admin pov !"
        else:
            not_admin = f"I'm not admin in <b>{update_chat_title}</b>! - Pov"
...
def user_can_ban(func):
    ...
    def user_is_banhammer(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if (
            ...
            and user not in [777000, 5242142514]
        ):
...
from AngelRobot.modules import connection
