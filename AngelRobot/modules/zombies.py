...
from AngelRobot import pbot
from AngelRobot.utils.admins import can_restrict
...
async def ban_zombies(_, message: Message):
    ...
    if len(deleted_u) > 0:
        ...
        return await cleaner.edit_text(
            f"Cleaned `{banned}` zombies is your group.\nFailed to remove `{failed}` aAdmin zombies."
        )
    else:
        return await check.edit_text(no_z)
...
__help__ = """
*Remove Deleted Accounts in your groups*
